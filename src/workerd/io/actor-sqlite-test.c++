...
struct ActorSqliteTest final {
  ...
  // Default to sync mode (Workerd), can be overridden if we want to test production behavior.
...
KJ_TEST("move-earlier operations wait for pending move-later operations in async mode") {
  ActorSqliteTest test;
  // Test async behavior (production mode).
  test.isSynchronous = false;
